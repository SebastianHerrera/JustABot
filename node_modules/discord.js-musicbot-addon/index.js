...
try {
    ...
    class Music {
      constructor(client, options) {
        ...
        this.play = {
          ...
          help: (options && options.play && options.play.help) || "Reproduce una cancion/playlist por url o comando.",
        ...
        this.help = {
          ...
          help: (options && options.help && options.help.help) || "Ayuda por los comandos. Pero... ¿Quien la necesita?",
        ...
        this.pause = {
          ...
          help: (options && options.pause && options.pause.help) || "Pus... Pausa la musica.",
        ...
        this.resume = {
          ...
          help: (options && options.resume && options.resume.help) || "Reanuda la musica",
        ...
        this.leave = {
          ...
          help: (options && options.leave && options.leave.help) || "Me sacas del chat de voz :( ",
        ...
        this.queue = {
          ...
          help: (options && options.queue && options.queue.help) || "Mira como va la cola",
        ...
        this.np = {
          ...
          help: (options && options.np && options.np.help) || "Muestra el texto que se reproduce ahora..",
        ...
        this.loop = {
          ...
          help: (options && options.loop && options.loop.help) || "Establece el estado del bucle para la cola..",
        ...
        this.search = {
          ...
          help: (options && options.search && options.search.help) || "Busca hasta 10 videos de YouTube.",
        ...
        this.clearqueue = {
          ...
          help: (options && options.clear && options.clear.help) || "Borra toda la cola.",
        ...
        this.volume = {
          ...
          help: (options && options.volume && options.volume.help) || "Cambia la salida de volumen del bot. Hablo fuerte? D:",
        ...
        this.remove = {
          ...
          help: (options && options.remove && options.remove.help) || "Eliminar una canción de la cola por posición en la cola.",
        ...
        this.skip = {
          ...
          help: (options && options.skip && options.skip.help) || "Salta una canción o canciones con `skip [número]``",
    ...
    musicbot.playFunction = (msg, suffix, args) => {
      if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('Error!', `No te encuentras en un canal de voz`));
      if (!suffix) return msg.channel.send(musicbot.note('Error', 'No se a expecificado el video.'));
      let q = musicbot.getQueue(msg.guild.id);
      if (q.songs.length >= musicbot.maxQueueSize && musicbot.maxQueueSize !== 0) return msg.channel.send(musicbot.note('Error', 'Error, maximo tamaño de la cola exedido!'));
      ...
      if (searchstring.startsWith('http') && searchstring.includes("list=")) {
        ...
        ytpl(playid, function(err, playlist) {
          if(err) return msg.channel.send(musicbot.note('fail', `Algo salió mal al buscar esa lista de reproducción!`));
          if (playlist.items.length <= 0) return msg.channel.send(musicbot.note('fail', `No se pudo obtener ningún video de esa lista de reproducción.`));
          if (playlist.total_items >= 50) return msg.channel.send(musicbot.note('fail', `demasiados videos para poner en cola. Se permite un máximo de 50.`));
          var index = 0;
          var ran = 0;
          ...
          playlist.items.forEach(video => {
            ...
            if (ran >= playlist.items.length) {
              if (index == 0) msg.channel.send(musicbot.note('fail', `No puedo obtener ninguna canción de esa lista de reproducción!`))
              else if (index == 1) msg.channel.send(musicbot.note('note', `En cola una canción. Te puedes esperar?`));
              else if (index > 1) msg.channel.send(musicbot.note('note', `En cola ${index} Canciones.`));
            }
          });
        });
      } else {
        msg.channel.send(musicbot.note("Buscar:", `\`Estoy buscando la canción:: ${searchstring}\`~`));
        new Promise(async (resolve, reject) => {
          let result = await musicbot.searcher.search(searchstring, { type: 'video' });
          resolve(result.first)
        }).then((res) => {
          if (!res) return msg.channel.send(musicbot.note("fail", "Auch! Algo ha salido mal, prova de nuevo! :("));
          ...
          if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
            ...
            try {
              embed.setAuthor('Añadido a la cola', client.user.avatarURL);
              ...
              embed.addField("Cola en:", res.queuedOn, musicbot.inlineEmbeds);
              ...
              if (musicbot.requesterName && resMem) embed.setFooter(`Cancion por: ${client.users.get(res.requester).username}`, res.requesterAvatarURL);
              if (musicbot.requesterName && !resMem) embed.setFooter(`Cancion por: \`UnknownUser (ID: ${res.requester})\``, res.requesterAvatarURL);
              msg.channel.send({
                embed
              });
            } catch (e) {
            ...
            try {
              ...
              .replace(/~/g, '\\~')
              .replace(/`/g, '\\`');
              msg.channel.send(`Ahora reproduciendo:: **${songTitle}**\Pedido por: ${client.users.get(res.requester).username}\nCola en: ${res.queuedOn}`)
            } catch (e) {
    ...
    musicbot.helpFunction = (msg, suffix, args) => {
      ...
      if (!suffix) {
        if (msg.channel.permissionsFor(msg.guild.me)
          ...
          embed.setDescription(`usa \`${musicbot.botPrefix}${musicbot.help.name} Nombre del comando\` para ayuda en el uso. Cualquier persona con un rol nombrado \`${musicbot.djRole}\` puede usar los comandos.`);
          ...
          var cmdmsg = `= Comandos de musica =\nUsa ${musicbot.botPrefix}${musicbot.help.name} [command] para ayuda en un comando. Cualquier persona con un rol nombrado \`${musicbot.djRole}\` Puede usar cualquier comando.\n`;
          let index = 0;
          let max = musicbot.commandsArray.length;
          for (var i = 0; i < musicbot.commandsArray.length; i++) {
        ...
        if (msg.channel.permissionsFor(msg.guild.me)
          ...
          if (command.exclude) return msg.channel.send(musicbot.note('fail', `${suffix} No es un comando valido!`));
        ...
        msg.channel.send(musicbot.note('fail', `${suffix} No es un comando valido`));
      };
    };
    ...
    musicbot.skipFunction = (msg, suffix, args) => {
      ...
      const queue = musicbot.getQueue(msg.guild.id);
      if (!musicbot.canSkip(msg.member, queue)) return msg.channel.send(musicbot.note('fail', `No puedes saltar esto porque no lo pusiste en la cola.`));

      if (musicbot.queues.get(msg.guild.id).loop == "song") return msg.channel.send(musicbot.note("fail", "No se puede omitir mientras que el bucle está configurado como simple."));
      ...
      dispatcher.end();
      msg.channel.send(musicbot.note("note", "Cancion saltada :D."));
    };

    musicbot.pauseFunction = (msg, suffix, args) => {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'NO se está reproduciendo musica.'));
      if (!musicbot.isAdmin(msg.member) && !musicbot.anyoneCanPause) return msg.channel.send(musicbot.note('fail', 'No puedes poner pausa.'));

      const dispatcher = voiceConnection.player.dispatcher;
      if (dispatcher.paused) return msg.channel.send(musicbot.note(`fail`, `Musica pausada :)!`))
    ...
    musicbot.leaveFunction = (msg, suffix) => {
      if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanLeave === true) {
        const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
        if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'No estoy en un canal de voz.'));
        musicbot.emptyQueue(msg.guild.id);

        if (!voiceConnection.player.dispatcher) return;
        voiceConnection.player.dispatcher.end();
        voiceConnection.disconnect();
        msg.channel.send(musicbot.note('note', 'Me he salido del canal de voz :c.'));
      } else {
        const chance = Math.floor((Math.random() * 100) + 1);
        if (chance <= 10) return msg.channel.send(musicbot.note('fail', `Me temo que no puedo dejar que hagas eso., ${msg.author.username}.`))
        else return msg.channel.send(musicbot.note('fail', 'Lo sentimos, no tienes permiso para hacer eso.'));
      }
    }
    ...
    musicbot.npFunction = (msg, suffix, args) => {
      ...
      if (musicbot.queues.get(msg.guild.id).songs.length <= 0) return msg.channel.send(musicbot.note('note', 'Cola en limpio.'));
      ...
      if (msg.channel.permissionsFor(msg.guild.me)
        ...
        try {
          ...
          if (musicbot.requesterName && resMem) embed.setFooter(`Canción por ${client.users.get(queue.last.requester).username}`, queue.last.requesterAvatarURL);
          if (musicbot.requesterName && !resMem) embed.setFooter(`Canción por \`UnknownUser (ID: ${queue.last.requester})\``, queue.last.requesterAvatarURL);
          msg.channel.send({
            embed
          });
        } catch (e) {
        ...
        try {
          var songTitle = queue.last.title.replace(/\\/g, '\\\\')
            ...
            .replace(/~/g, '\\~')
            .replace(/`/g, '\\`');
          msg.channel.send(`Ahora suena: **${songTitle}**Canción por: ${client.users.get(queue.last.requester).username}\Cola en: ${queue.last.queuedOn}`)
        } catch (e) {
          console.error(`[${msg.guild.name}] [npCmd] ` + e.stack);
        };
      }
    ...
    musicbot.queueFunction = (msg, suffix, args) => {
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note("fail", "No se pudo encontrar una cola para este servidor."));
      else if (musicbot.queues.get(msg.guild.id).songs.length <= 0) return msg.channel.send(musicbot.note("fail", "Cola limpia :D."));
      const queue = musicbot.queues.get(msg.guild.id);
      if (suffix) {
        let video = queue.songs.find(s => s.position == parseInt(suffix) - 1);
        if (!video) return msg.channel.send(musicbot.note("fail", "No pude encontrar el video."));
        const embed = new Discord.RichEmbed()
        .setAuthor('Canción en cola', client.user.avatarURL)
        ...
        .addField("COla en", video.queuedOn, musicbot.inlineEmbeds)
        .addField("Pocision", video.position + 1, musicbot.inlineEmbeds);
        ...
        if (musicbot.requesterName && resMem) embed.setFooter(`Cancion pory ${client.users.get(video.requester).username}`, video.requesterAvatarURL);
        if (musicbot.requesterName && !resMem) embed.setFooter(`Canción por \`UnknownUser (ID: ${video.requester})\``, video.requesterAvatarURL);
    ...
    musicbot.searchFunction = (msg, suffix, args) => {
      if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Como quieres escuchar musica si no estás en un canal de voz?~`));
      if (!suffix) return msg.channel.send(musicbot.note('fail', 'No has especificado un video!'));
      const queue = musicbot.getQueue(msg.guild.id);
      if (queue.songs.length >= musicbot.maxQueueSize && musicbot.maxQueueSize !== 0) return msg.channel.send(musicbot.note('fail', 'Tamaño maximo de la cola excedido!'));

      let searchstring = suffix.trim();
      msg.channel.send(musicbot.note('search', `Estoy buscando: \`${searchstring}\``))
        .then(response => {
          musicbot.searcher.search(searchstring, {
              type: 'video'
            })
            .then(searchResult => {
              if (!searchResult.totalResults || searchResult.totalResults === 0) return response.edit(musicbot.note('fail', 'No he podido obtener resultados :c.'));

              const startTheFun = async (videos, max) => {
                if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
                  const embed = new Discord.RichEmbed();
                  embed.setTitle(`Escoge tu musica`);
                  embed.setColor(musicbot.embedColor);
                  var index = 0;
                  ...
                  .then(firstMsg => {
                    ...
                    .then(collected => {
                      ...
                      if (mcon === "Cancelar") return firstMsg.edit(musicbot.note('note', 'Busqueda cancelada.'));
                      ...
                      if (song_number >= 0) {
                        ...
                        embed.addField("Cola en", videos[song_number].queuedOn, musicbot.inlineEmbeds);
                        ...
                        if (musicbot.requesterName && resMem) embed.setFooter(`Cancio por ${client.users.get(videos[song_number].requester).username}`, videos[song_number].requesterAvatarURL);
                        if (musicbot.requesterName && !resMem) embed.setFooter(`Cancio por \`UnknownUser (ID: ${videos[song_number].requester})\``, videos[song_number].requesterAvatarURL);
                        msg.channel.send({
                          embed
                        }).then(() => {
                  ...
                  msg.channel.send(`\`\`\`\n= Seleciona tu cancion =\n${vids}\n\n= Escribe "cancel" para cancelar  =`).then(firstMsg => {
                    var filter = null;
                    if (max === 0) {
                      filter = m => m.author.id === msg.author.id &&
                      m.content.includes('1') ||
                    ...
                    .then(collected => {
                      ...
                      if (mcon === "cancel") return firstMsg.edit(musicbot.note('note', 'Busqueda cancelada.'));
                      const song_number = parseInt(mcon) - 1;
                      if (song_number >= 0) {
                        firstMsg.delete();
                        ...
                        if (musicbot.bigPicture) embed.setImage(`https://img.youtube.com/vi/${videos[song_number].id}/maxresdefault.jpg`);
                        const resMem = client.users.get(videos[song_number].requester);
                        if (musicbot.requesterName && resMem) embed.setFooter(`Cancion por ${client.users.get(videos[song_number].requester).username}`, videos[song_number].requesterAvatarURL);
                        if (musicbot.requesterName && !resMem) embed.setFooter(`Cancion por \`UnknownUser (ID: ${videos[song_number].requester})\``, videos[song_number].requesterAvatarURL);
                        msg.channel.send({
                          embed
                        }).then(() => {
                          queue.songs.push(videos[song_number]);
    ...
    musicbot.volumeFunction = (msg, suffix, args) => {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'No se está reproduciendo música.'));
      if (!musicbot.canAdjust(msg.member, musicbot.queues.get(msg.guild.id))) return msg.channel.send(musicbot.note('fail', `Sólo los administradores o DJ pueden cambiar el volumen.`));
      const dispatcher = voiceConnection.player.dispatcher;

      if (!suffix || isNaN(suffix)) return msg.channel.send(musicbot.note('fail', 'Sin volumen especificado.'));
      suffix = parseInt(suffix);
      if (suffix > 200 || suffix <= 0) return msg.channel.send(musicbot.note('fail', 'Volumen fuera de rango, debe estar dentro de 1 a 200'));

      dispatcher.setVolume((suffix / 100));
      musicbot.queues.get(msg.guild.id).volume = suffix;
      msg.channel.send(musicbot.note('note', `Volumen cambiado a ${suffix}%.`));
    };

    musicbot.clearFunction = (msg, suffix, args) => {
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note("fail", "Nunguna cola para este server."));
      if (!musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note("fail", `Sólo los administradores o personas con el ${musicbot.djRole} puede borrar colas.`));
      musicbot.emptyQueue(msg.guild.id).then(res => {
        msg.channel.send(musicbot.note("note", "Cola limpiada."));
        const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
        if (voiceConnection !== null) {
          const dispatcher = voiceConnection.player.dispatcher;
          if (!dispatcher || dispatcher === null) {
            if (musicbot.logging) return console.log(new Error(`dispatcher null on skip cmd [${msg.guild.name}] [${msg.author.username}]`));
            return msg.channel.send(musicbot.note("fail", "Algo salió mal. Comunicate con Beleño"));
          };
          if (voiceConnection.paused) dispatcher.end();
          dispatcher.end();
        }
        ...
        return msg.channel.send(musicbot.note("fail", "Algo salió mal limpiando la cola. Comunicate con Beleño"));
      })
    };
    ...
    musicbot.executeQueue = (msg, queue) => {
      ...
      new Promise((resolve, reject) => {
          ...
          if (!video) {
            ...
            if (!video) {
              msg.channel.send(musicbot.note('note', 'Canciones reproducidas! '));
              musicbot.emptyQueue(msg.guild.id);
              const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
              if (voiceConnection !== null) return voiceConnection.disconnect();
            }
                ...
                if (musicbot.queues.get(msg.guild.id).songs.length > 0) {
                  if (loop == "none" || loop == null) {
                    ...
                    musicbot.updatePositions(musicbot.queues.get(msg.guild.id).songs, msg.guild.id).then(res => {
                      musicbot.queues.get(msg.guild.id).songs = res;
                      musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
                    }).catch(() => { console.error(new Error("algo salió mal moviendo la cola")); });
                  } else if (loop == "queue" || loop == "song") {
                    musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
                  };
                } else if (musicbot.queues.get(msg.guild.id).songs.length <= 0) {
                  if (msg && msg.channel) msg.channel.send(musicbot.note('note', 'Reproducción terminada.'));
                  musicbot.queues.set(msg.guild.id, {songs: [], last: null, loop: "none", id: msg.guild.id, volume: musicbot.defVolume});
                  if (musicbot.musicPresence) musicbot.updatePresence(musicbot.queues.get(msg.guild.id), msg.client, musicbot.clearPresence).catch((res) => { console.warn(`[MUSIC] Problem updating MusicPresence`); });
                  const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
                  if (voiceConnection !== null) return voiceConnection.disconnect();
